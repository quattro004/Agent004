...
- ⚠️ **Zod 4 peer dependency conflict** may still apply — `@strands-agents/sdk` and its transitive dependencies may require different Zod major versions (some packages still on Zod 3 vs Zod 4). npm 7+ strict peer dep enforcement can cause install errors. **Preferred resolution**: use npm `overrides` in root `package.json` to force a single Zod 4 version (e.g., `"overrides": { "zod": "^4.3.6" }`). Zod 4 ships a `zod/v3` compatibility mode (`z.setCompatMode(true)`) for gradual migration of Zod 3 consumers. Avoid `--legacy-peer-deps` in production workflows — it silently masks dependency conflicts and should only be used as a last resort. Latest stable Zod version: **4.3.6** (April 2026).
